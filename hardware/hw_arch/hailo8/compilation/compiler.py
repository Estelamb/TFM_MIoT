        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if dataset_key:
                try:
                    ...
                    logger.info(f"[Hailo8] Downloading dataset zip from datasets/{dataset_key}...")
                    ...
                    logger.info(f"[Hailo8] Extracting dataset zip...")
                    await asyncio.to_thread(_extract_zip, str(zip_path), str(extract_dir))
                    
                    # Prepare calibration images
                    logger.info(f"[Hailo8] Preparing calibration images...")
                    ...
                    if valid_count > 0:
                        ...
                        logger.info(f"[Hailo8] Prepared {valid_count} calibration images from dataset.")
                    else:
                        logger.warning("[Hailo8] No valid images were successfully processed from the dataset. Falling back to dummy images.")
                except Exception as e:
                    logger.warning(f"[Hailo8] Failed to prepare calibration images from dataset: {e}. Falling back to dummy images.")
            else:
                logger.warning("[Hailo8] dataset_key is empty or not provided. Falling back to dummy images.")

            if not dataset_prepared:
                logger.info("[Hailo8] Generating dummy calibration images...")
            ...
            try:
                ...
                if proc.returncode != 0:
                    ...
                    logger.error(f"[Hailo8] Docker compilation failed:\n{err}")
