...
export const getDevices   = () => api.get<Device[]>("/api/devices").then(r => r.data);
export const createDevice = (body: { name: string; hardware_type: string; description?: string }) => api.post<Device>("/api/devices", body).then(r => r.data);
...
export const getModels   = () => api.get<Model[]>("/api/models").then(r => r.data);
export const deleteModel = (id: string) => api.delete(`/api/models/${id}`);
export async function uploadModel(
  name: string, description: string, hardware_type: string,
  file: File, compile = true
): Promise<Model> {
...
export interface Script {
  ...
  hardware_type: string; script_sha256?: string; created_at: string;
}
export const getScripts   = () => api.get<Script[]>("/api/scripts").then(r => r.data);
...
export const getDeployments       = () => api.get<Deployment[]>("/api/deployments").then(r => r.data);
export const getDeviceDeployments = (device_id: string) =>
  api.get<Deployment[]>(`/api/deployments/device/${device_id}`).then(r => r.data);
export const createDeployment = (body: { device_id: string; model_id: string; script_id: string }) =>
  api.post<Deployment>("/api/deployments", body).then(r => r.data);
...
export interface DeviceState {
  ...
  active_deployment_id: string; last_seen_at: string;
}
export interface InferenceResult {
  timestamp: string; deployment_id: string; result_json: string;
}
export const getMonitoringStates = () => api.get<DeviceState[]>("/api/monitoring/devices").then(r => r.data);
export const getDeviceState      = (id: string) => api.get<DeviceState>(`/api/monitoring/devices/${id}`).then(r => r.data);
export const getInferenceResults = (id: string, limit = 20) =>
  api.get<InferenceResult[]>(`/api/monitoring/devices/${id}/inference?limit=${limit}`).then(r => r.data);
