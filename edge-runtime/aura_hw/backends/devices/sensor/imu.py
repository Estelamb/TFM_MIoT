...
class GeneralIMUSensor(SensorBackend):
    """
    General IMU Sensor library/wrapper.
    Dynamically loads custom IMU sensor from hardware/sensors/imu/<driver>/library.py.
    """
    ...
    def open(self, params: dict) -> None:
        logger.info(f"[GeneralIMUSensor] Dynamically loading custom IMU driver '{self._driver}'")
        cls = load_component_class("sensors", "imu", self._driver)
        ...
        if hasattr(self._delegate, "initialize"):
            ...
            if not success:
                raise OSError(f"Failed to initialize custom IMU driver '{self._driver}'")
    ...
    def measure(self) -> dict:
        if not self._is_open or self._delegate is None:
            raise RuntimeError(f"IMU sensor '{self.component_id}' is not open.")
        ...
        if hasattr(self._delegate, "read_value"):
            ...
            raise AttributeError(f"Custom IMU driver class has no read/measure method")
