...
class GeneralBuzzerActuator(ActuatorBackend):
    """
    General Buzzer Actuator library/wrapper.
    Dynamically loads custom buzzer actuator from hardware/actuators/buzzer/<driver>/library.py.
    """
    ...
    def open(self, params: dict) -> None:
        logger.info(f"[GeneralBuzzerActuator] Dynamically loading custom buzzer driver '{self._driver}'")
        cls = load_component_class("actuators", "buzzer", self._driver)
        ...
        if hasattr(self._delegate, "initialize"):
            ...
            if not success:
                raise OSError(f"Failed to initialize custom buzzer driver '{self._driver}'")
    ...
    def write(self, value: Any) -> None:
        if not self._is_open or self._delegate is None:
            raise RuntimeError(f"Buzzer actuator '{self.component_id}' is not open.")
        ...
        if hasattr(self._delegate, "write_value"):
            ...
            raise AttributeError(f"Custom buzzer driver class has no write/write_value method")
