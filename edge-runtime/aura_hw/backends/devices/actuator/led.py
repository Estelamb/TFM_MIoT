...
class GeneralLEDActuator(ActuatorBackend):
    """
    General LED Actuator library/wrapper.
    Dynamically loads custom LED actuator from hardware/actuators/led/<driver>/library.py.
    """
    ...
    def open(self, params: dict) -> None:
        logger.info(f"[GeneralLEDActuator] Dynamically loading custom LED driver '{self._driver}'")
        cls = load_component_class("actuators", "led", self._driver)
        ...
        if hasattr(self._delegate, "initialize"):
            ...
            if not success:
                raise OSError(f"Failed to initialize custom LED driver '{self._driver}'")
    ...
    def write(self, value: Any) -> None:
        if not self._is_open or self._delegate is None:
            raise RuntimeError(f"LED actuator '{self.component_id}' is not open.")
        ...
        if hasattr(self._delegate, "write_value"):
            ...
            raise AttributeError(f"Custom LED driver class has no write/write_value method")
